...
class MSMT17(BaseImageDataset):
    ...
    def _process_dir(self, dir_path, list_path):
        ...
        for img_info in tqdm.tqdm(lines):
        #     pbar = tqdm.tqdm(lines, unit='batch', leave=True)
            img_path, pid = img_info.split(' ')
            pid = int(pid)  # no need to relabel
            camid = int(img_path.split('_')[2])
            img_path = osp.join(dir_path, img_path.replace('.jpg', '.png'))
            ...
            dataset.append((img_path,  self.pid_begin +pid, camid-1, 1, mos - 1, 1))
