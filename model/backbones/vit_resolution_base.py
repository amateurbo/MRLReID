...
class MyBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.qkv_res = nn.Linear(dim, dim * 3, bias=qkv_bias)
    ...
    def forward(self, x_res, x_id):
        B, N, C = x_res.shape
        qkv_res = self.qkv(x_res).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_id = self.qkv(x_id).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_res, k_res, v_res = qkv_res[0], qkv_res[1], qkv_res[2]   # make torchscript happy (cannot use tensor as tuple)
        q_id, k_id, v_id = qkv_id[0], qkv_id[1], qkv_id[2]   # make torchscript happy (cannot use tensor as tuple)
        q, k, v = q_res, k_id, v_id
        x = x_id
        x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks_id2 = Block(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0.1, norm_layer=norm_layer)

        self.blocks_resolution2 = Block(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0.1, norm_layer=norm_layer)
