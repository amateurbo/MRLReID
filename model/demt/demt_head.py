...
class DemtHead(nn.Module):
    ...
    def forward(self, inp, inp_shape=None, **kwargs):
...
if __name__ == '__main__':
    ...
    img1 = torch.rand([2, 768])
    img1 = rearrange(img1, 'i j -> i j () ()')
    img1 = rearrange(img1, 'i j () () -> i j')
    # img11 = torch.unsqueeze(img1, -1)
    # img111 = torch.unsqueeze(img11, -1)
    img2 = torch.rand([2, 768, 1, 1])
    feat = head([img1, img2])

    for item in feat:
        print(feat[item].shape)
        # print(sum(feat[item] == img1))
