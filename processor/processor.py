...
def do_train(cfg, args,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # for n_iter, data in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                if cfg.DATALOADER.SAMPLER == 'softmax2_triplet':
                    ...
                    score, feat, res_score, part_score, part_feat = model(img, target, cam_label=target_cam, view_label=target_view)
                    # print('target_cam: {}'.format(target_cam))
                    # print('res_target: {}'.format(res_target))
                    loss = loss_fn(score, feat, target, target_cam, res_score, res_target, part_score, part_feat)
                elif cfg.DATALOADER.SAMPLER == 'softmax2_triplet2':
                    score, feat, part_score, part_feat = model(img, target, cam_label=target_cam, view_label=target_view)
                    # print('target_cam: {}'.format(target_cam))
                    # print('res_target: {}'.format(res_target))
                    loss = loss_fn(score, feat, target, target_cam, part_score, part_feat)
        ...
        if (epoch >= start_eval) and (epoch % eval_period == 0):
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    model.eval()
    ...
    return maxrank1, maxmap, maxrank1_epoch, maxmAP_epoch
...
def do_inference(cfg,
    ...
    if cfg.TEST.EVAL_RESOLUTION:
        ...
        pre_res = []
        tar_res = []
        for n_iter, (img, vid, camid, camids, target_view, imgpath, target, _) in enumerate(tqdm.tqdm(val_loader)):
            with torch.no_grad():
                img = img.to(device)
                camids = camids.to(device)
                # target = vid.to(device)
                ...
                # pre_res.append(resolution_score.tolist())
                # tar_res.append(target.tolist())
                for pre, tar, path in zip(resolution_score, target, imgpath):
                    pre_res.append(pre.item())
                    tar_res.append(tar.item())
                    img_path_list.append(path)
                    dif = pre.item() - tar.item()
                    dif_list.append(dif)
                    dif_dict[dif] = [path, pre.item(), tar.item()]
            # if n_iter > 8:
            #     break
        dic = {'pre': pre_res, 'tar': tar_res, 'path': img_path_list, 'dif': dif_list}
        f = open('atmp.txt', 'w')
        f.write(str(dic))
        f.close()

        f = open('adif_dict.txt', 'w')
        f.write(str(dif_dict))
        f.close()
        return
        #         pre_mean = torch.mean(resolution_score)
        #         tar_mean = torch.mean(target.float())
        ...
        #         print("PLCC:{}".format(suma / torch.sqrt(sumb1 * sumb2)))
